...
- **Testing `areaType` via `setSupportedAreas`:** spy `areaService.setSupportedAreas`, extract `mock.calls[last][1]` as `ServiceArea.Area[]`, assert `areas[0]?.areaInfo.locationInfo.areaType`. Import `ServiceArea` from `matterbridge/matter/clusters` and `CommonAreaNamespaceTag` from `matterbridge/matter`.
...
- **SkipArea progress helpers:** unit-test exported `markAreaSkipped`/`getNextPendingArea`/`computeEstimatedEndTime` in `serviceAreaHandler.test.ts`; multi-room no-info path keeps `selectedAreas`, sets `currentArea` to first — update `platformRunner.test.ts` if stale.
