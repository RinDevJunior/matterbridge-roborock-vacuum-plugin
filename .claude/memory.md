...
- `ModeUtils.assertModeChange` (matter.js `@matter/node` mode-base) never checks operational state for Run Mode or Clean Mode — only "is newMode supported." No SDK/plugin layer gates `changeToMode` by state; mid-clean mode changes already work with zero guards anywhere in the chain.
- `ProgressStruct.estimatedTime` (ServiceArea) spec: "SHOULD NOT change... except when repopulating the entire instance" — set once at Progress-entry creation, unlike `estimatedEndTime` which re-publishes every tick. No per-room historical duration data exists anywhere (confirmed again Jul 2026); only the active area can get a non-null estimate, reusing the session-level `clean_time`/`clean_percent` formula.
...
- **buildProgressUpdate tests:** new exported function; when `areaId === activeAreaId` and entry is newly created, set `estimatedTime: estimatedTimeForActiveArea ?? null` only on active area (no `estimatedTime` key on non-active entries). Existing entries never overwritten (spec: "SHOULD NOT change"); status transitions preserve `estimatedTime`. Test via `buildProgressUpdate(existing, selectedAreas, activeAreaId, estimatedTimeForActiveArea)` with mock arrays.
- **computeAreaEstimatedTime tests:** new pure helper; returns raw remaining seconds (duration, not epoch) using same `computeRemainingSeconds` formula as session-level `computeEstimatedEndTimeFromCleanProgress` but without `now +` offset. Test: `cleanPercent=25, cleanTime=60` → `180`; `cleanPercent=100` → `0`; validates bounds (5–100%), rejects `cleanPercent=undefined` → `null`. Import from `src/share/estimatedEndTime.js`.
- **createDefaultRvcCleanModeClusterServer tests:** override declares `RvcCleanMode.Feature.DirectModeChange` via `MatterbridgeRvcCleanModeServer.with(...)`. Test defaults: `currentMode=1`, three modes (Vacuum/1, Mop/2, DeepClean/3) per SDK defaults. Verify method returns `this` for chaining. No operational-state guard needed (SDK `changeToMode` already allows mid-clean mode changes). Import `RvcCleanMode` in test file.

## Common Pitfalls

<!-- Things to avoid — bugs found, anti-patterns, footguns -->

- Plan contracts can be internally inconsistent (e.g. specify a param signature that contradicts the stated behavior) — implementer following the plan literally can still ship an unused-param diagnostic; reviewer must verify plan intent, not just conformance.
