...
- `handleCleaningWithoutInfo`'s old V1-shadowing bug (4be5ff7) was fixed Jul 22 (8d8c84f, before rc02 tag) via reorder, but its `selectedAreas[0]` pin (line 309) still fires unconditionally whenever `cleaningInfo` is transiently absent mid-run (confirmed real on V1/V10 S8, not just B01) — alternates with `resolveAreaFromCleaningInfo`'s correct segment resolution, producing a `0,X,0,X...` `currentArea` flicker (log-confirmed `rc02` regression). Fix: reuse last-known `ServiceArea.currentArea` attribute instead of pinning to `selectedAreas[0]` when no fresh resolution exists.
- `resolveAreaFromCleaningInfo`'s `if (!mappedArea)` (`serviceAreaHandler.ts:402`) is a falsy-zero check — a legitimate `areaId = 0` resolution is wrongly treated as "not found" and `currentArea` gets cleared to `null`. Use `mappedArea === undefined` (matches `RoomIndexMap.getAreaId`/`getAreaIdV2`'s `number | undefined` contract).
