...
- `progress` reset on start-of-clean (Jul 14, 2026): 3rd trigger added — `handleServiceAreaUpdate` compares stored-vs-current `operationalState` "cleaning session active" classification (Docked/Stopped/Error = not active), NOT raw `CLEANING_STATES` membership (misses `ReturningDock`/`EmptyingDustContainer` detours → false positives). Flag stored in `AreaManagementService` (proxied via `RoborockService`), not a module-level var.
- `isActivelyCleaning*` family renamed to `CleaningSession*` (Sep 2026, pure rename): `isCleaningSessionActiveState()`, `setLastCleaningSessionActiveState`/`getLastCleaningSessionActiveState` (`roborockService.ts`/`areaManagementService.ts`). Name reflects real semantics: "session active, possibly mid-detour," not "vacuuming right now."
...
- `handleServiceAreaUpdate`'s idle-reset branch (`serviceAreaHandler.ts:187`, Sep 2026) checks raw `message.state === OperationStatusCode.Idle || OperationStatusCode.IdleDockedQ10` (105) — NOT resolved `operationalState`, which falls back to `Docked` for ANY unrecognized code (confirmed live: Q10 code `102` occurs mid-clean yet also resolves to Docked via that fallback). Only explicitly-vetted codes are treated as idle, to avoid false-clearing `selectedAreas`/`progress`/`currentArea` during an active clean.
- `deviceConfigurator.ts`'s `registerAreasListener` callback (Sep 2026) no longer force-nulls `currentArea` on every invocation — it fired on every B01 live map push (~1-2s), wiping `currentArea` almost immediately after `resolveAreaFromCleaningInfo` set it correctly. `currentArea`'s lifecycle is now owned solely by `serviceAreaHandler.ts` (idle branch + `handleActiveMapChanged`).
...
- **AreaManagementService.startPeriodicRefresh tests (Sep 2026):** fire-and-forget immediate `triggerMapInfoRefresh(duid)` call at t=0 plus 5-minute interval. Use `await vi.advanceTimersByTimeAsync(0)` to flush pending promises and confirm immediate call; `advanceTimersByTimeAsync(5001)` for interval tick. Two back-to-back `startPeriodicRefresh` calls → 2 immediate + 1 interval (third call replaces first, both immediate calls survive). Error logging happens for both immediate + interval rejections.
...
- **Q10 live-position coordinate transform tests (`roomMatrixResolver.test.ts`):** pose in Roborock-common mm (+25500 offset) maps to grid pixel via `(pose - OFFSET) / resolution + origin`; synthetic grid fixtures (254×280=71120 bytes filled with 243 sentinel) with specific room bytes at critical indices; real-capture validation: pose `{27920, 28220}`, origin `{149, 209, 50mm/px}` resolves to pixel (197, 155) with byte value 40 (roomId=10). Offset subtraction is critical — old test values that worked pre-offset need recalibration (e.g., {0,0} → {25500,25500} to hit same pixel).
- **handleServiceAreaUpdate idle-detection tests (Sep 19, 2026):** idle branch checks raw `message.state` against `OperationStatusCode.Idle`/`IdleDockedQ10` directly — do NOT mock `operationalState`/`robot.getAttribute` to force this branch (rejected approach: unsafe, since the resolved-`operationalState` Docked fallback applies to any unrecognized code, including ones confirmed active mid-clean like raw `102`). Set `message.state: OperationStatusCode.IdleDockedQ10` (105) directly for the non-canonical-idle regression case.
