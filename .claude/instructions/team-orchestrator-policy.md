...
- **low** — ≤3 files, an existing pattern to follow, no cross-module uncertainty. Docs/config always low.
- **medium** — genuine design uncertainty: >3 files, no clear pattern, or touch points that need checking before coding.
- **high** — cross-module/layer, unclear entry points, new feature area, architecture change.

When in doubt between low and medium, start low — direct-executor reports "bigger than it looked" and the task restarts as medium.
...
| Investigation only    | architect                                                                |
| Low                   | direct-executor (lite, default)                                          |
| Medium                | architect → approval → implementer → reviewer → test-writer → documenter |
...
Frontmatter is source of truth — never pass `model:` except `implementer` on high (`"sonnet"`). Haiku: compiler, documenter, finalizer, wiki-manager, test-writer, implementer(default). Sonnet: technical-architect, investigator, reviewer, direct-executor, release-manager. Never upgrade unless user asks or subagent reports blocked.
...
Track per cycle: `ta_id`, `implementer_id`, `reviewer_id`, `tw_id`, `documenter_id`, `compiler_id`, `finalizer_id`, `release_id`, `executor_id`. wiki-manager/investigator nest under architect, no EM label.
