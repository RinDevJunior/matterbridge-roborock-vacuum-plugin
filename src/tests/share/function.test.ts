...
describe('share/function helpers', () => {
  ...
  describe('state_to_matter_operational_status', () => {
    it('returns Running for cleaning/mapping states', () => {
      expect(state_to_matter_operational_status(OperationStatusCode.Cleaning)).toBe(
        RvcOperationalState.OperationalState.Running,
      );
      expect(state_to_matter_operational_status(OperationStatusCode.Mapping)).toBe(
        RvcOperationalState.OperationalState.Running,
      );
      expect(state_to_matter_operational_status(OperationStatusCode.RoomClean)).toBe(
        RvcOperationalState.OperationalState.Running,
      );
    });

    it('returns Error for error states', () => {
      expect(state_to_matter_operational_status(OperationStatusCode.InError)).toBe(
        RvcOperationalState.OperationalState.Error,
      );
      expect(state_to_matter_operational_status(OperationStatusCode.ChargingError)).toBe(
        RvcOperationalState.OperationalState.Error,
      );
      expect(state_to_matter_operational_status(OperationStatusCode.Unknown)).toBe(
        RvcOperationalState.OperationalState.Error,
      );
    });

    it('returns Paused for paused state', () => {
      expect(state_to_matter_operational_status(OperationStatusCode.Paused)).toBe(
        RvcOperationalState.OperationalState.Paused,
      );
    });

    it('returns Stopped for shutting down', () => {
      expect(state_to_matter_operational_status(OperationStatusCode.ShuttingDown)).toBe(
        RvcOperationalState.OperationalState.Stopped,
      );
    });

    it('returns SeekingCharger for docking-related states', () => {
      expect(state_to_matter_operational_status(OperationStatusCode.ReturnToDock)).toBe(
    ...
    it('returns Docked for idle/charging states (default)', () => {
      expect(state_to_matter_operational_status(OperationStatusCode.Idle)).toBe(
        RvcOperationalState.OperationalState.Docked,
      );
      expect(state_to_matter_operational_status(OperationStatusCode.Sleeping)).toBe(
        RvcOperationalState.OperationalState.Docked,
      );
      expect(state_to_matter_operational_status(OperationStatusCode.Charging)).toBe(
        RvcOperationalState.OperationalState.Docked,
      );
