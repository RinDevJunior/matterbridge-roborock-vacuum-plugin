...
import { RoomMapInfoListener } from '../../../../../roborockCommunication/routing/listeners/implementation/roomMapInfoListener.js';
...
describe('RoomMapInfoListener', () => {
  let logger: AnsiLogger;
  let listener: RoomMapInfoListener;
  let handler: AbstractMessageHandler;
  const duid = 'test-duid';

  beforeEach(() => {
    logger = createMockLogger();
    listener = new RoomMapInfoListener(duid, logger);
    handler = createMockHandler();
  });

  describe('constructor', () => {
    it('should set name and duid correctly', () => {
      expect(listener.name).toBe('RoomMapInfoListener');
  ...
  describe('onMessage - early returns', () => {
    ...
    it('should log error and return early when no handler is registered', () => {
      const message = makeRpcResponseMessage(duid, baseResultBody);
      listener.onMessage(message);
      expect(logger.error).toHaveBeenCalledWith('[RoomMapInfoListener]: No handler registered');
      expect(handler.onBatteryUpdate).not.toHaveBeenCalled();
    });
    ...
    it('should log debug and return early when message does not contain state', () => {
      ...
      listener.onMessage(message);
      expect(logger.debug).toHaveBeenCalledWith(expect.stringContaining('does not contain state'));
      expect(handler.onBatteryUpdate).not.toHaveBeenCalled();
    });
  });
