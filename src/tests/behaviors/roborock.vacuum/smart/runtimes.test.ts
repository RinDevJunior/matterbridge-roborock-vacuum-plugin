...
describe('getCurrentCleanModeSmart', () => {
  ...
  it('returns the correct key if an exact CleanSetting match exists', () => {
    ...
    expect(smartModeResolver.resolve(new CleanModeSetting(VacuumSuctionPower.Balanced, MopWaterFlow.Medium, 0, MopRoute.Standard, CleanSequenceType.Persist))).toBe(5);
  });

  it('returns 12 for MopMax', () => {
    expect(smartModeResolver.resolve(new CleanModeSetting(VacuumSuctionPower.Off, MopWaterFlow.High, 0, MopRoute.Standard, CleanSequenceType.Persist))).toBe(32);
  });

  it('returns 16 for Vacuum Default if waterFlow is Off', () => {
    expect(smartModeResolver.resolve(new CleanModeSetting(VacuumSuctionPower.Balanced, MopWaterFlow.Off, 0, MopRoute.Standard, CleanSequenceType.Persist))).toBe(66);
  });

  it('returns 5 for Vac & Mop Default if neither suctionPower nor waterFlow is Off', () => {
    expect(smartModeResolver.resolve(new CleanModeSetting(VacuumSuctionPower.Balanced, MopWaterFlow.Medium, 1, MopRoute.Standard, CleanSequenceType.Persist))).toBe(5);
  });

  it('returns 5 if no match and no fallback applies', () => {
    expect(smartModeResolver.resolve(new CleanModeSetting(999, 999, 999, 999, CleanSequenceType.Persist))).toBe(CleanModeLabelInfo[CleanModeDisplayLabel.MopAndVacuumDefault].mode); // should return MopAndVacuumCustom mode
  });
});
