...
describe('runtimes.getCurrentCleanModeDefault', () => {
  it('returns 10 when any value is Custom', () => {
    const s = new CleanModeSetting(VacuumSuctionPower.Custom, MopWaterFlow.Off, 0, MopRoute.Standard, CleanSequenceType.Persist);
    expect(defaultModeResolver.resolve(s)).toBe(CleanModeLabelInfo[CleanModeDisplayLabel.MopAndVacuumEnergySaving].mode);
  });

  it('matches exact preset from CleanSetting', () => {
    // Use a known mapping from CleanSetting: mode 5 exists as default Vac & Mop
    const s = new CleanModeSetting(VacuumSuctionPower.Balanced, MopWaterFlow.Medium, 0, MopRoute.Standard, CleanSequenceType.Persist);
  ...
  it('returns 31 for mop default', () => {
    const s = new CleanModeSetting(VacuumSuctionPower.Off, MopWaterFlow.Medium, 0, MopRoute.Standard, CleanSequenceType.Persist);
  ...
  it('returns 66 for vacuum default', () => {
    const s = new CleanModeSetting(VacuumSuctionPower.Balanced, MopWaterFlow.Off, 0, MopRoute.Standard, CleanSequenceType.Persist);
  ...
  it('returns 5 if no CleanSetting match and not covered by other rules', () => {
    ...
    const s = new CleanModeSetting(999, 888, 0, 777, CleanSequenceType.Persist);
    expect(defaultModeResolver.resolve(s)).toBe(CleanModeLabelInfo[CleanModeDisplayLabel.MopAndVacuumDefault].mode); // because none are Off, should return MopAndVacuumCustom mode
  });
});
