...
describe('runtimes.getCurrentCleanModeDefault', () => {
  it('returns 10 when any value is Custom', () => {
    ...
    expect(defaultModeResolver.resolve(s)).toBe(
      CleanModeLabelInfo[CleanModeDisplayLabel.VacuumAndMopEnergySaving].mode,
    );
  });
  ...
  it('returns 5 for vac & mop default when both on', () => {
    ...
    expect(defaultModeResolver.resolve(s)).toBe(CleanModeLabelInfo[CleanModeDisplayLabel.VacuumAndMopDefault].mode);
  });
  ...
  it('returns 5 if no CleanSetting match and not covered by other rules', () => {
    ...
    expect(defaultModeResolver.resolve(s)).toBe(CleanModeLabelInfo[CleanModeDisplayLabel.VacuumAndMopDefault].mode); // because none are Off, should return MopAndVacuumCustom mode
  });
});
