...
function createMockPlatform(
	...
	return asPartial<RoborockMatterbridgePlatform>({
		...
		roborockService: asPartial<RoborockService>({
			...
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(false),
			setLastCleaningSessionActiveState: vi.fn(),
...
describe('handleServiceAreaUpdate with progress', () => {
	...
	it('should handle progress when no prior progress exists on clean start', async () => {
		...
		const mockRoborockService = asPartial<RoborockService>({
			...
			getProgress: vi.fn().mockReturnValue([]),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(true),
			setLastCleaningSessionActiveState: vi.fn(),
		});
...
describe('handleServiceAreaUpdate without cleaning_info — B01 live position fallback', () => {
	let robot: RoborockVacuumCleaner;

	beforeEach(() => {
		vi.clearAllMocks();
		robot = createMockRobot('test-duid-b01-position', 100);
		// Set protocol to B01 via device mock
		setReadOnlyProperty(
			robot,
			'device',
			asPartial<Device>({ duid: 'test-duid-b01-position', pv: ProtocolVersion.B01 }),
		);
	});

	it('should keep currentArea null and NOT call requestV1MapRefresh when B01 cache returns undefined', async () => {
		const selectedAreas = [1, 2];
		...
		const mockRoborockService = asPartial<RoborockService>({
			getSelectedAreas: vi.fn().mockReturnValue(selectedAreas),
			getSupportedAreasIndexMap: vi.fn().mockReturnValue(indexMap),
			getB01ResolvedRoom: vi.fn().mockReturnValue(undefined), // No cached room
			requestV1MapRefresh: vi.fn(), // Should NOT be called for B01
			getProgress: vi.fn().mockReturnValue([]),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(false),
			setLastCleaningSessionActiveState: vi.fn(),
		...
		// Assert requestV1MapRefresh was NOT called (B01 has no refresh mechanism)
...
describe('handleServiceAreaUpdate — start-of-clean progress reset', () => {
	...
	it('should reset progress when transitioning from Docked to Running', async () => {
		...
		const mockRoborockService = asPartial<RoborockService>({
			...
			getProgress: vi.fn().mockReturnValue([]),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(false), // was not cleaning
			setLastCleaningSessionActiveState: vi.fn(),
		});
		...
		expect(vi.mocked(mockRoborockService.setLastCleaningSessionActiveState).mock.calls[0]).toEqual([
	...
	it('should not reset progress when still cleaning (no transition)', async () => {
		...
		const mockRoborockService = asPartial<RoborockService>({
			...
			getProgress: vi.fn().mockReturnValue(initialProgress),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(true), // already was cleaning
			setLastCleaningSessionActiveState: vi.fn(),
		});
	...
	it('should not reset progress during mid-clean detour (e.g., Paused)', async () => {
		...
		const mockRoborockService = asPartial<RoborockService>({
			...
			getProgress: vi.fn().mockReturnValue([]),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(true), // was actively cleaning
			setLastCleaningSessionActiveState: vi.fn(),
		});
	...
	it('should not reset progress when transitioning between two actively-cleaning states (e.g., SeekingCharger)', async () => {
		...
		const mockRoborockService = asPartial<RoborockService>({
			...
			getProgress: vi.fn().mockReturnValue([]),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(true), // was cleaning before detour
			setLastCleaningSessionActiveState: vi.fn(),
		});
	...
	it('should store lastCleaningSessionActiveState on every call, regardless of transition', async () => {
		// Arrange: verify that setLastCleaningSessionActiveState is called on EVERY invocation
		const mockRoborockService = asPartial<RoborockService>({
			getSupportedAreas: vi.fn().mockReturnValue([]),
			getSelectedAreas: vi.fn().mockReturnValue([1]),
			getProgress: vi.fn().mockReturnValue([]),
		...
		// Assert: setLastCleaningSessionActiveState must be called with the current isCleaningSessionActive value
		expect(vi.mocked(mockRoborockService.setLastCleaningSessionActiveState)).toHaveBeenCalled();
	...
	it('should detect transition when moving from Stopped to Running', async () => {
		...
		const mockRoborockService = asPartial<RoborockService>({
			...
			getProgress: vi.fn().mockReturnValue([]),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(false), // was stopped
			setLastCleaningSessionActiveState: vi.fn(),
		});
	...
	it('should detect transition when moving from Error to Running', async () => {
		...
		const mockRoborockService = asPartial<RoborockService>({
			...
			getProgress: vi.fn().mockReturnValue([]),
			setProgress: vi.fn(),
			getLastCleaningSessionActiveState: vi.fn().mockReturnValue(false), // was in error
			setLastCleaningSessionActiveState: vi.fn(),
		});
