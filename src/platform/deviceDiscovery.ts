...
export class DeviceDiscovery {
	...
	public async discoverDevices(): Promise<boolean> {
		...
		if (cleanModeSettings) {
			this.log.notice(
				`Custom Clean Mode Mapping Enabled: ${this.configManager.isCustomCleanModeMappingEnabled ? 'Enabled' : 'Disabled'},
		...
		this.roborockService = new RoborockService(
			{
				...
				refreshInterval: this.configManager.refreshInterval,
				baseUrl,
				persist,
		...
		for (const device of devices) {
			const isDeviceAllowed = this.configManager.isDeviceAllowed({ duid: device.duid });
