...
export class L01Serializer implements Serializer {
  ...
  private generateKey(localKey: string, timestamp: number): Buffer<ArrayBufferLike> {
  ...
  private generateInitializationVector(timestamp: number, nonce: number, sequence: number): Buffer<ArrayBufferLike> {
  ...
  private generateAAD(timestamp: number, nonce: number, sequence: number, connectNonce: number, ackNonce: number): Buffer<ArrayBufferLike> {
