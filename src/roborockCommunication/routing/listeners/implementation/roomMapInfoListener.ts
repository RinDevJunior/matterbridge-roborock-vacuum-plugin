...
export class RoomMapInfoListener implements AbstractMessageListener {
  readonly name = 'RoomMapInfoListener';
  ...
  public onMessage(message: ResponseMessage): void {
    if (message.duid !== this.duid) {
      this.logger.debug(`[RoomMapInfoListener]: Message DUID ${message.duid} does not match listener DUID ${this.duid}`);
      return;
    }

    if (!this.handler) {
      this.logger.error(`[RoomMapInfoListener]: No handler registered`);
      return;
    }

    if (!message.isForProtocols([Protocol.general_request, Protocol.rpc_response])) {
      this.logger.debug(`[RoomMapInfoListener]: Message not for general_request or rpc_response protocol`);
      return;
    }
