...
export const B01VacuumModeResolver = {
	...
	resolveMopMode(waterFlow: number): number {
		switch (waterFlow) {
	...
	resolveCleanRoute(mopRoute: number): number {
		switch (mopRoute) {
	...
	resolveQ7CleanMode(suctionPower: number, waterFlow: number): number {
	...
	resolveQ10CleanMode(suctionPower: number, waterFlow: number): number {
		// mop only, return 3,
	...
	resolveMopModeWithDistanceOff(waterFlow: number, distance_off: number): number {
		if (distance_off > 0) {
			return 4; // Customize with distance off
		}

		return B01VacuumModeResolver.resolveMopMode(waterFlow);
	},
};
