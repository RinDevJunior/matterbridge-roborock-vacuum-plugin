...
3. **Spawn `technical-architect`** (explain mode) per `.cursor/instructions/team-orchestrator-policy.md` — resume `ta_id` only if it still holds context on the same area; otherwise spawn fresh.
