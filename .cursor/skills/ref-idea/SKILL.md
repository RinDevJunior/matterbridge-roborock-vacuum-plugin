...
3. **Create the task folder** — `docs/<short-task-description>/requirement.md` with `type: explain` and this boilerplate baked in:
   ...
   - Serena/CodeGraph work on this repo only — use Grep/Glob/Read inside reference workspaces (read-only).
...
4. **Spawn `technical-architect`** (explain mode) per `.cursor/instructions/team-orchestrator-policy.md` — resume `ta_id` only if it still holds context on the same topic; otherwise spawn fresh.
