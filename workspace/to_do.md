...
- [x] **Fix B01/Q10 map parser for Roborock Q10 S5+ (protocol 301, LZ4)** — added marker-byte classifier to route Q10 unencrypted LZ4 payloads to new decompressor/parser; Q7 AES+zlib path untouched; best-effort room-name extraction based on python-roborock reference (unconfirmed against real hardware); new distinguishable error message for future log captures
...
- [ ] **Q10 S5+ map parser real-device validation** — ask affected user to share plugin log after updating to confirm whether best-effort room-name extraction works or surfaces new "Q10 map binary parse failed" error (would indicate field-layout guess needs real raw hex capture for correction)
- [ ] **Q10 "trace/path" secondary payload variant (deferred)** — Q10 also sends a different smaller sub-format via protocol 301; explicitly deferred as separate follow-up task
